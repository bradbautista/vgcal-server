...
years.forEach((year, i) => {
  console.log(
    `Scheduling refresh of ${year} releases for ${
      hour > 12 ? hour - 12 + i : hour + i
    }:${minute} ${hour > 11 ? 'p.m.' : 'a.m.'} `
  );
  ...
  console.log(
    `Scheduling transmission of backup of ${year} releases for ${
      hour > 12 ? hour - 12 + i : hour + i
    }:${minute} ${hour > 11 ? 'p.m.' : 'a.m.'} and 15 seconds`
  );

  cron.schedule(`15 ${minute} ${hour + i} * * *`, function () {
    console.log(
      `It's ${hour > 12 ? hour - 12 + i : hour + i}:${minute} ${
        hour > 11 ? 'p.m.' : 'a.m.'
      }, e-mailing daily backup of records for ${year} to ${EMAILUSER}`
    );
